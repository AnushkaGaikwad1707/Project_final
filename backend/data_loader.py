...
def get_component_analysis(component_id: str) -> Optional[dict]:
    ...
    # 3. Historical measurements (0h, 24h from holdout, 96h/168h where available)
    historical_measurements = []
    if component_id in holdout_input_df.index:
        h_row = holdout_input_df.loc[component_id]
        for e in [0, 24]:
            meas = {"epoch_h": e}
            for p in PARAMS:
...
def search_components(query: str, limit: int = 50) -> List[dict]:
    ...
    if not q:
        return []
    mask = fusion_df["component_id"].str.upper().str.contains(q, na=False)
    if "lot_id" in fusion_df.columns:
        mask = mask | fusion_df["lot_id"].str.upper().str.contains(q, na=False)
...
def get_evaluation_metrics() -> dict:
    ...
    headline = {
        "scope": "holdout_168h_baseline",
        "description": "Frozen production release operating at 1% FPR budget",
        "tp": 65,
        "tn": 1240,
        "fp": 13,
        "fn": 25,
    ...
    # 2. Relaxed threshold operating point comparison (25 -> 24 FN, 13 -> 18 FP)
    relaxed = {
        "scope": "holdout_168h_relaxed_cutoff",
        "description": "Relaxed operating threshold capturing +1 defect at the cost of +5 false alarms",
        "tp": 66,
        "tn": 1235,
        "fp": 18,
        "fn": 24,
        ...
        "passed": 1259,
        "recall": 66 / 90, # 0.7333
        "precision": 66 / (66 + 18), # 0.7857
        "specificity": 1235 / 1253, # 0.9856
        "fpr": 18 / 1253, # 0.0144
        "accuracy": (66 + 1235) / 1343, # 0.9687
        "f1": (2 * 66) / (2 * 66 + 18 + 24), # 0.7586
        "f2": (5 * 66) / (5 * 66 + 18 + 4 * 24), # 0.7432
    }

    comparison = {
        "fn_delta": -1, # 25 -> 24 (caught 1 more defect)
        "fp_delta": 5,  # 13 -> 18 (incurred 5 more false positives)
        "tp_delta": 1,  # 65 -> 66
        "recall_delta": 0.0111, # +1.11%
        "precision_delta": -0.0476, # -4.76%
        "f2_delta": 0.0012, # +0.12%
        "engineering_rationale": "Trading 5 false alarms for 1 caught defect does not meaningfully improve F2 utility (+0.12%); model was frozen at the baseline 1% FPR budget."
    }

    # 3. Evolution across burn-in epochs
    epochs = [
        ...
        "stages": [
            {
                ...
                "description": "Whole-lot holdout split (LOTSPLIT-05) isolating 6 lots per variant (CMOS_A, CMOS_B, CMOS_C) with zero leakage.",
            ...
            },
            {
                ...
                "description": "Cross-module corroboration enforcing CONFIRMED -> REJECT, early forecast breaches -> REJECT, statistical drift -> MONITOR, compliant -> PASS.",
                "status": "SYNTHESIZED"
            },
            {
                "id": 6,
                ...
                "description": "High-performance in-memory analytics server providing instant search, deep-dive evaluation, and operational isolation.",
                "status": "ONLINE"
            },
            {
                "id": 7,
