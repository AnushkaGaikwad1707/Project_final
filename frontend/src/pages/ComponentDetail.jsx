...
export default function ComponentDetail() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [result, epochRes] = await Promise.all([
        api.get(`/api/analysis/${componentId}`),
        api.get(`/api/analysis/${componentId}/module-a`).catch(() => ({ epochs: {} }))
      ]);
      setData(result);
      setEpochsData(epochRes?.epochs || {});
    } catch (err) {
      setError(err);
    } finally {
      setLoading(false);
  ...
  const parameters = [
    ...
    { key: 'Input_Leakage_Current', label: 'Input Leakage Current', unit: 'nA' },
    { key: 'Active_Supply_Current', label: 'Active Supply Current', unit: 'mA' },
          ...
          <div className="verdict-banner-desc">
            {finalVerdict === 'REJECT' && (
              tier === 'CONFIRMED' 
                ...
                : `Prognostic forecast indicates parametric wearout breach beyond static upper specification limits at 168h.`
            )}
            ...
            {finalVerdict === 'PASS' && (
              `Both Module A static screening and Module B 168h wearout forecasts confirm compliant operational behavior within normal envelopes.`
            )}
          </div>
        </div>
      </div>
      ...
      <div className="detail-grid">
        ...
        <Card title="Module A: Static Screening (168h)" className="module-card">
          <div className="score-section">
            ...
            <div className="score-scale-hint">
              ...
              <span>0.90 Cutoff Floor</span>
              <span>1.00 Confirmed</span>
            </div>
          </div>
        ...
        <Card title="Multi-Epoch Screening Evolution (Module A)" className="module-card">
          ...
          <div className="epoch-prog-note">
            ...
            <span>Demonstrates risk progression from initial wafer test (0h) to final burn-in completion (168h).</span>
          </div>
        </Card>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="param-stats">
                ...
                <div className="stat">
                  <span className="stat-label">Datasheet Spec</span>
                  <span className="stat-val code-font">{limit != null ? Number(limit).toFixed(2) : 'None'}</span>
                </div>
              </div>
        ...
        <div className="historical-section">
          <div className="section-header">
            <div>
              ...
              <p className="section-subtitle">Observed sensor measurements across initial qualification burn-in epochs</p>
            </div>
          </div>
